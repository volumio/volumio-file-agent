...
export const SyncAdapter = (db: Database): SyncAdapter => {
  ...
  return {
    ...
    getAllMediaFilesByArtist: (artistName) => {
      try {
        const records = statements.getAllMediaFilesByArtist.all({
          artist: artistName,
        }) as MediaFileRecord[]
    ...
    getAllMediaFilesByGenre: (genreName) => {
      try {
        const records = statements.getAllMediaFilesByGenre.all({
          genre: genreName,
        }) as MediaFileRecord[]
    ...
    getAllMediaFilesByYear: (year) => {
      try {
        const records = statements.getAllMediaFilesByYear.all({
          year,
        }) as MediaFileRecord[]
    ...
    getAllMediaFilesHavingAlbum: () => {
      try {
        const records = statements.getAllMediaFilesHavingAlbum.all() as MediaFileRecord[]

        return right(records.map(mediaFileRecordToPortMediaFile))
      } catch (error) {
        return left('PERSISTENCY_FAILURE')
      }
    },
