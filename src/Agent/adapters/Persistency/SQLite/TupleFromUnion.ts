...
type TupleHead<Tuple extends any[]> = Tuple extends [
  infer HeadElement,
  ...unknown[]
]
  ? HeadElement
...
type TupleTail<Tuple extends any[]> = ((...args: Tuple) => never) extends (
  a: any,
  ...args: infer TailElements
) => never
  ? TailElements
  : never
...
type TuplePrepend<Tuple extends any[], NewElement> = ((
  h: NewElement,
  ...t: Tuple
) => any) extends (...r: infer ResultTuple) => any
  ? ResultTuple
  : never
